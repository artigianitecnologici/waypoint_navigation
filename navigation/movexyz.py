...
if __name__ == '__main__':
    try:
        ...
        while not rospy.is_shutdown():
            ...
            rospy.loginfo('Aspetto Comandi cmd,x,y,z,nomefile')
            
            data = rospy.wait_for_message('/move', String)
            rospy.loginfo('Received file '+ data.data)
            mycmd = data.data
            rospy.loginfo('Received file '+ mycmd )
            valori = mycmd.split(",")
            mycmd = valori[0]
            posx = float(valori[1])
            posy = float(valori[2])
            ...
            path_waypoint =  "/home/ubuntu/src/waypoint_navigation/waypoints/" + nomefile  + ".csv"
            ...
            if ( mycmd == 'START'): 
                ...
                with open(path_waypoint, 'w'  ) as file:
                    ...
                    file.write(str(posx)+","+str(posy)+","+str(posz) + "," + "0" + "\n")
                    file.close()
